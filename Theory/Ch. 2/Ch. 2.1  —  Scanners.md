...
An example of a token type would be the `Token::Ident(std::string)` variant inside of the `Token` enum class, where `Ident` is the actual token, and `std::string` is the type of value that can be stored in the token.
